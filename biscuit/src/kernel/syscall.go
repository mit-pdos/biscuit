	...
	*chtf = *ptf
	...
	if mkproc {
		...
		if !ok {
			...
			// proc_t.terminate which will clean everything up. the
...
func segload(proc *proc_t, entry int, hdr *elf_phdr, fops fdops_i) err_t {
...
func (e *elf_t) elf_load(proc *proc_t, f *fd_t) (int, int, int, bool) {
	PT_LOAD := 1
	PT_TLS  := 7
	istls := false
	tlssize := 0
